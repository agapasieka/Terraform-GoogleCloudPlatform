...
3. Test the app in terminal 
  ...
  ``` 
  ![Hello form Docker](hfd.png)

4. Test the app in browser
  ```sh
  localhost:8080
  ``` 
 ![Hello form Docker](hfd2.png)
...
Before we create any resources with terraform, verify the region defianed in provider.tf file. I used 'europe-west1' but you can use any region of your choice. 

To deploy to App Engine, we need to zip our code first and place it in the storage bucket. 

1. In deploy-app-to-gcp/ locate the bucket.tf file, uncomment the code and save the file. 
  Make sure you are in deploy_apps_to_gcp directory.
...
1. Deploy GKE cluster and VPC, uncomment the code in vpc.tf and gke.tf
